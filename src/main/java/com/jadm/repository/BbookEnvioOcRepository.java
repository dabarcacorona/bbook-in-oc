...
public interface BbookEnvioOcRepository extends CrudRepository<BbookEnvioOc, String> {
	...
	@Query(value = "select distinct pmg_po_number from app_sam.sdi_sdipmghde where tran_type = 'A' and   pmg_stat_code = 4 and   download_date_1 is null and  pmg_ext_po_num in (select distinct t.pmg_ext_po_num from app_sam.BBOOK_SDIPMGDTI t)", nativeQuery = true)
